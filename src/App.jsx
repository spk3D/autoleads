...
<button
onClick={() => setV(“form”)}
style={{ padding: “8px 12px”, borderRadius: “8px”, background: v === “form” ? “#c8a96e” : “#1a1a1a”, color: v === “form” ? “#000” : “#fff”, border: “none”, cursor: “pointer”, fontSize: “13px” }}
>
➕ Ajouter
</button>
<button
onClick={() => setV(“leads”)}
style={{ padding: “8px 12px”, borderRadius: “8px”, background: v === “leads” ? “#c8a96e” : “#1a1a1a”, color: v === “leads” ? “#000” : “#fff”, border: “none”, cursor: “pointer”, fontSize: “13px” }}
>
📋 Leads
</button>
</div>
...
```
  ...
  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px", marginBottom: "20px" }}>
    {[
      ...
      { l: "Propriétaires", val: ow.length, c: "#7eb8c8" },
      ...
      <div style={{ display: "flex", background: "#111", borderRadius: "10px", padding: "4px", marginBottom: "16px" }}>
        {[["client", "🚗 Client"], ["owner", "🔑 Propriétaire"]].map(([k, l]) => (
          <button
            key={k}
            onClick={() => setT(k)}
            style={{ flex: 1, padding: "10px", border: "none", borderRadius: "8px", cursor: "pointer", background: t === k ? (k === "client" ? "#c8a96e" : "#7eb8c8") : "transparent", color: t === k ? "#000" : "#555", fontWeight: 600, fontSize: "13px" }}
          >
            {l}
          </button>
        ))}
      ...
      {t === "client" ? (
        <div style={{ background: "#111", borderRadius: "12px", padding: "16px" }}>
          ...
          {[
            ...
            ["TÉLÉPHONE *", "tel", cf.telephone, (e) => setCf({ ...cf, telephone: e })],
            ["VILLE", "text", cf.ville, (e) => setCf({ ...cf, ville: e })],
            ["DATE SOUHAITÉE", "date", cf.date, (e) => setCf({ ...cf, date: e })],
          ].map(([l, tp, val, fn]) => (
          ...
          <div style={{ marginBottom: "10px" }}>
            <div style={{ fontSize: "11px", color: "#888", fontWeight: 600, marginBottom: "4px" }}>VÉHICULE</div>
        ...
        <div style={{ background: "#111", borderRadius: "12px", padding: "16px" }}>
          <div style={{ color: "#7eb8c8", fontWeight: 600, marginBottom: "12px" }}>🔑 Nouveau Propriétaire</div>
          {[
            ["NOM *", of.nom, (e) => setOf({ ...of, nom: e })],
            ["TÉLÉPHONE *", of.telephone, (e) => setOf({ ...of, telephone: e })],
            ["MARQUE", of.marque, (e) => setOf({ ...of, marque: e })],
            ["MODÈLE", of.modele, (e) => setOf({ ...of, modele: e })],
            ["ANNÉE", of.annee, (e) => setOf({ ...of, annee: e })],
          ].map(([l, val, fn]) => (
            <div key={l} style={{ marginBottom: "10px" }}>
              <div style={{ fontSize: "11px", color: "#888", fontWeight: 600, marginBottom: "4px" }}>{l}</div>
          ...
          <div style={{ marginBottom: "14px" }}>
            <div style={{ fontSize: "11px", color: "#888", fontWeight: 600, marginBottom: "4px" }}>DISPONIBILITÉ</div>
  ...
  {v === "leads" && (
    !auth ? (
      <div style={{ background: "#111", borderRadius: "14px", padding: "28px", textAlign: "center" }}>
        ...
        <div style={{ fontSize: "16px", fontWeight: 700, color: "#c8a96e", marginBottom: "8px" }}>Accès protégé</div>
        <div style={{ fontSize: "13px", color: "#555", marginBottom: "20px" }}>Entrez votre mot de passe</div>
        <input
          type="password"
          style={{ ...inp, textAlign: "center", letterSpacing: "4px", marginBottom: "10px" }}
          value={pw}
          onChange={(e) => setPw(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && (pw === P ? (setAuth(true), setErr(false)) : setErr(true))}
        />
        {err && <div style={{ color: "#f66", fontSize: "12px", marginBottom: "10px" }}>Mot de passe incorrect</div>}
        <button
          onClick={() => { if (pw === P) { setAuth(true); setErr(false); } else { setErr(true); } }}
          style={{ width: "100%", padding: "12px", background: "#c8a96e", color: "#000", border: "none", borderRadius: "10px", fontWeight: 700, cursor: "pointer" }}
        >
          Accéder
        </button>
      </div>
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px", alignItems: "center" }}>
          ...
          <button
            ...
            style={{ background: "none", border: "none", color: "#f66", cursor: "pointer", fontSize: "13px" }}
          >
            Déconnexion
          </button>
        </div>
        ...
        {[
          { title: "🚗 Clients", data: cl, del: delC, wa: (c) => "https://wa.me/" + c.telephone + "?text=" + encodeURIComponent("Bonjour " + c.nom + ", suite à votre demande de location...") },
          { title: "🔑 Propriétaires", data: ow, del: delO, wa: null },
        ].map((sec) => (
              ...
              sec.data.map((x) => (
                <div key={x.id} style={{ background: "#111", borderRadius: "10px", padding: "12px", marginBottom: "8px", border: "1px solid #1a1a1a" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                    ...
                    <button
                      onClick={() => sec.del(x.id)}
                      style={{ background: "none", border: "none", color: "#f66", cursor: "pointer", fontSize: "16px" }}
                    >
                      🗑
                    </button>
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                    {x.telephone && (
                      <span style={{ background: "rgba(37,211,102,0.1)", border: "1px solid rgba(37,211,102,0.2)", borderRadius: "6px", padding: "3px 8px", fontSize: "12px", color: "#25d366" }}>
                        📞 {x.telephone}
                      </span>
                    )}
                    {x.ville && (
                      <span style={{ background: "#222", borderRadius: "6px", padding: "3px 8px", fontSize: "12px", color: "#888" }}>
                        📍 {x.ville}
                      </span>
                    )}
                    {x.vehicule && (
                      <span style={{ background: "rgba(200,169,110,0.1)", border: "1px solid rgba(200,169,110,0.2)", borderRadius: "6px", padding: "3px 8px", fontSize: "12px", color: "#c8a96e" }}>
                        🚗 {x.vehicule}
                      </span>
                    )}
                    {x.marque && (
                      <span style={{ background: "rgba(126,184,200,0.1)", borderRadius: "6px", padding: "3px 8px", fontSize: "12px", color: "#7eb8c8" }}>
                        🚘 {x.marque} {x.modele}
                      </span>
                    )}
                    {x.disponibilite && (
